...
def getMaxWords(stringPassed, numberOfTopWords):
	...
	for i in range(len(splitString)):
		...
		if(wordOn.lower() not in frequencyList and lettersInString(wordOn) > int(len(wordOn) / 2)):
...
def getESPNNews(leagueName, numberNews):
	...
	while(numberLeft > 0 and articlesPassed < len(newsArticles)):
		...
		if("www.espn.com/"+leagueName+"/story/" in linkRef and "Matchups, schedules" not in newsHeadline):
			if('(' in newsHeadline):
				newsHeadline = newsHeadline.split('(')[0]
			articleDoc["title"] = newsHeadline
			articleDoc["url"] = linkRef
			articleBody = getArticleBody(linkRef)
			articleDoc["snippet"] = articleBody
			articleDoc["trending"] = numberToShow - numberLeft + 1 + numberAlready
			articleDoc["source"] = "ESPN"
			articleDoc["summary"] = summarizeArticle(articleBody)
			'''
			...
			articleJSON += "&nbsp;<br>"
			'''
			articleDB.insert_one(articleDoc)
			numberLeft -= 1
		articlesPassed += 1
	return articleJSON
